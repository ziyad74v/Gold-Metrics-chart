...
function saveTimeframeToStorage(intervalSec) {
    localStorage.setItem(TIMEFRAME_KEY, String(intervalSec));
}

function loadMaVisibilityFromStorage() {
    const raw = localStorage.getItem(MA_VISIBLE_KEY);
    if (raw === null) return true;
    return raw === "1";
}

function saveMaVisibilityToStorage(visible) {
    localStorage.setItem(MA_VISIBLE_KEY, visible ? "1" : "0");
}

function loadAlertPriceFromStorage() {
    const raw = localStorage.getItem(ALERT_PRICE_KEY);
    const parsed = Number(raw);
...
function saveAlertPriceToStorage(price) {
    if (price === null) {
        localStorage.removeItem(ALERT_PRICE_KEY);
    } else {
        localStorage.setItem(ALERT_PRICE_KEY, String(price));
    }
}

function loadAlertEnabledFromStorage() {
    return localStorage.getItem(ALERT_ENABLED_KEY) === "1";
}

function saveAlertEnabledToStorage(enabled) {
    localStorage.setItem(ALERT_ENABLED_KEY, enabled ? "1" : "0");
}
...
async function initSeasonalsFromHistory() {
    try {
        const cached = localStorage.getItem(SEASONALS_CACHE_KEY);
        let rows = null;
        if (cached) {
        ...
        if (!rows) {
            ...
            if (rows.length > 0) {
                localStorage.setItem(SEASONALS_CACHE_KEY, JSON.stringify({ savedAt: Date.now(), rows }));
            }
        }
